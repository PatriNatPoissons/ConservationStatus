...
length(unique(Status_IUCN$Species))

spp_matrice <- unique(Status_IUCN$Species)
spp_fr <- unique(elasmo_FR$Species)
setdiff(spp_matrice, spp_fr)
...
Status_IUCN$Last_status <- factor(Status_IUCN$Last_status ,levels = c("NA", "NE", "DD","LC", "NT", "VU", "EN", "CR", "RE", "3"))
...
status <- ggplot(data = IUCN_summary_EU, aes(x = Last_status, y= Nb.species, fill = as.factor(Groupe))) + 
    ...
    labs(title = paste("Dernier statut IUCN par Region et par groupe d'espèces"), y = "Nombre d'espèces", 
...
### 4.1.2 All : temporal evolution of IUCN status
#### Sankey plot global
```{r sankey_global}
...
world_alluvial <- ggplot(IUCN_sankey_world, 
  ...
  labs(title = "Evolution des statuts mondiaux IUCN", subtitle = "Raies et requins présents en France", y = "Nb. d'espèces évaluées", x= "",
       caption = "Note : les dates d'évaluations varient selon les espèces") +
  scale_x_discrete(expand = c(0,0)) +
  scale_y_continuous(breaks=seq(0, 98, 10), sec.axis = sec_axis(~., breaks = seq(0, 90, 10)))
ggsave("figs\\IUCN_worldalluvial.png", width = 8, height = 11, units = "cm")
```
...
ggplot(IUCN_sankey_world, 
  ...
  theme(legend.position = "none",
        ...
        plot.subtitle = element_text(hjust = .5),
        plot.margin=unit(c(0.1,0.1,0.1,0.1), 'cm')) +
  labs(title = "Evolution des statuts UICN en Méditerranée", subtitle = "Raies et requins présents en France", y = "Nb. d'espèces évaluées", x= "", caption = "Note : évaluations de 2006/2007 et 2015/2016") +
  scale_x_discrete(expand = c(0,0)) +
  scale_y_continuous(breaks=seq(0, 98, 10), sec.axis = sec_axis(~., breaks = seq(0, 90, 10)))
ggsave("figs\\IUCN_med_alluvial_all_v2.png", width = 8, height = 11, units = "cm")
```
...
ggplot(IUCN_sankey_world, 
  ...
  geom_alluvial(flow.alpha = .6) +
  geom_alluvial_text(size = 2, color = "black") +
  scale_fill_manual(values = IUCN_palette[1:9]) +
  theme_alluvial(base_size = 8) +
  labs(x = NULL) +
  theme(legend.position = "none",
        plot.title = element_text(hjust = .5),
        ...
        plot.subtitle = element_text(hjust = .5),
        plot.margin=unit(c(0.1,0.1,0.1,0.1), 'cm')) +
  labs(title = "Evolution des statuts UICN dans l'Atlantique NE", subtitle = "Raies et requins présents en France", y = "Nb. d'espèces évaluées", x= "", caption = "Note : évaluations de 2006/2007 et 2015/2016") +
  scale_x_discrete(expand = c(0,0)) +
  scale_y_continuous(breaks=seq(0, 98, 10), sec.axis = sec_axis(~., breaks = seq(0, 90, 10)))
ggsave("figs\\IUCN_nea_alluvial_all_v2.png", width = 8, height = 11, units = "cm")
```

### 4.1.3 Sharks : temporal evolution of IUCN status
#### World
...
#save
ggsave("figs\\IUCN_med_sharks_alluvial.png", width = 8, height = 12, units = "cm")
```
...
### 4.1.4 Rays : Temporal evolution of IUCN status
#### World
```{r iucn_rays}
#Rays
...
heatmap_df$Level[heatmap_df$Level == "Regional"] <- "Regional"

#order in geog decreasing
heatmap_df$Level <- factor(heatmap_df$Level, levels = c("Int", "Cont", "Regional", "National"))
...
ggplot(heatmap_df, aes(Region, Species)) +
  ...
  labs(y = "", title = "Derniers statuts UICN", subtitle = "Espèces classées par statut français (UICN & MNHN 2013)") +
  guides(fill=guide_legend(title="Dernier statut")) +
  scale_y_discrete(labels=c("Squatina squatina"=expression(bold("Squatina squatina")), 
                            "Rostroraja alba"=expression(bold("Rostroraja alba")), 
                            "Dipturus intermedius"=expression(bold("Dipturus intermedius")),
                            parse=TRUE))

#export
ggsave("figs\\heatmap_all3.png", width = 16, height  = 23, units = "cm")

#do the same but only for two species per class for the pptx presentation
species_pptx <- c("Galeorhinus galeus", "Leucoraja fullonica", 
                  "Scyliorhinus canicula", "Dasyatis pastinaca", 
                  "Prionace glauca",
                  "Raja clavata", "Cetorhinus maximus", 
                  "Lamna nasus", "Mobula mobular",
...
Priority_IUCN <- Status_IUCN %>% filter(Region != "France") %>% 
  ...
  filter(! Family %in% c("Chimaeridae", "Rhinochimaeridae")) %>%
  rename(Status_FR = Cat_liste.rouge.France) %>%
  mutate_all(as.character) %>%
  mutate(Region = as.character(Region)) %>%
  select(Status_FR, Groupe, Species, Name_FR, Region, Last_status, Status_evolution) %>%
  ...
  drop_na(Façade) %>%
  group_by(Façade, Groupe, Species, Name_FR, Status_FR) %>%
  summarise(Nb.CR = sum(Last_status == "CR", na.rm = T),
            ...
            Tot = length(unique(Species)),
            Tot.2.evals = length(unique(Species[is.na(Status_evolution) == FALSE]))) %>%
...
#count nb of spp with at least 1 CR in one façade
Priority_IUCN_count <- Priority_IUCN %>% filter(Nb.CR >= 1)
table(Priority_IUCN_count$Façade, Priority_IUCN_count$Nb.CR, Priority_IUCN_count$Groupe)
length(unique(Priority_IUCN_count$Species[Priority_IUCN_count$Groupe == "Raies"]))
length(unique(Priority_IUCN_count$Species[Priority_IUCN_count$Groupe == "Requins"]))
#export
...
nb.vu.en.nt <-  Status_IUCN %>% filter(Region != "France") %>% 
  ...
  select(Status_FR, Species, Name_FR, Region, Last_status, Status_evolution) %>%
  group_by(Region) %>%
...
Priority_IUCN_large <- Status_IUCN %>% filter(Region != "France") %>% 
  left_join(.,elasmo_FR[c("Species", "Cat_liste.rouge.France", "Groupe")]) %>%
  filter(! Family %in% c("Chimaeridae", "Rhinochimaeridae")) %>%
  rename(Status_FR = Cat_liste.rouge.France) %>%
  mutate_all(as.character) %>%
  select(Status_FR, Groupe, Species, Name_FR, Region, Last_status, Status_evolution) %>%
  group_by(Groupe, Region) %>%
  summarise(Nb.CR = sum(Last_status == "CR", na.rm = T),
            Nb.VU.EN = sum(Last_status == "EN" |
                                Last_status == "VU" |
                                Last_status == "3", na.rm = T),
  ...
  filter(Region %in% c("Mediterranean Sea", "NE Atlantic", "World"))

#export
vroom_write(Priority_IUCN_large, "data\\Priority_world_med_nea_groupes.xlsx")

Priority_IUCN_large_all <- Status_IUCN %>% filter(Region != "France") %>% 
  left_join(.,elasmo_FR[c("Species", "Cat_liste.rouge.France", "Groupe")]) %>% 
  filter(! Family %in% c("Chimaeridae", "Rhinochimaeridae")) %>%
  rename(Status_FR = Cat_liste.rouge.France) %>%
  mutate_all(as.character) %>%
...
#export
vroom_write(Priority_IUCN_large_all, "data\\Priority_world_med_nea_all.xlsx")
```
...
for(i in 1:length(levels_iucn)){
  ...
  mutate(Level = ifelse(Region == c("Baltic Sea", "Mediterranean Sea", "NE Atlantic"), "Regional", "National")) %>%
...
freq <- freq %>% mutate(Region = as.character(Region)) %>%
  ...
  arrange(factor(Status_FR, levels = c(c("NA", "NE", "DD","LC", "NT", "VU", "EN", "CR", "RE"))), factor(Level, levels = c("National", "Regional", "Continental", "Mondial")), Region)
...
heatmap_all_third <- ggplot(FC1_POSH_ICCAT_ICES, aes(Region, Species)) +
  ...
  scale_y_discrete(labels=c("Squatina squatina"=expression(bold("Squatina squatina")), "Rostroraja alba"=expression(bold("Rostroraja alba")), parse=TRUE))#+
  #scale_y_discrete(sec.axis = sec_axis(~ ., name = "Statuts : France 2013", breaks = NULL, labels = NULL))  # NOT WORKING

#export
#ggsave("figs\\Evaluations_hors_UICN_v4.png", width = 16, height = 14, units = 'cm')
...
# 6. Stats pour l'abstract
## 6.1. Espèces par Region marine
```{r}
#atlantic
